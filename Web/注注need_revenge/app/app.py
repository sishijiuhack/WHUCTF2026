...
def store_user_snapshot(user: dict | None):
    if not user:
        session.pop("user_snapshot", None)
        return
    session["user_snapshot"] = {
        "id": user["id"],
...
def init_db():
    admin_password = os.environ.get("ADMIN_PASSWORD", "Admin#2026!Rabbit_it_is_a_good_day")
    admin_hash = encrypt_password(admin_password)
    schema = """
...
def current_user():
    ...
    try:
        ...
        if user:
            store_user_snapshot(user)
        return user
    except pymysql.MySQLError:
        app.logger.warning("current_user lookup failed, falling back to session snapshot")
        snapshot = session.get("user_snapshot")
        if snapshot and snapshot.get("id") == user_id:
            return snapshot
        return None
...
def update_avatar(user_id: int, avatar_path: str):
    ...
    snapshot = session.get("user_snapshot")
    if snapshot and snapshot.get("id") == user_id:
        snapshot["avatar"] = avatar_path
        session["user_snapshot"] = snapshot
...
def list_gallery(page: int):
    ...
    for path in IMAGE_DIR.iterdir():
        ...
        if path.is_file() or path.is_symlink():
            try:
                # Use lstat so uploaded symlinks are listed without depending on target metadata.
                stat = path.lstat()
            except OSError:
                continue
            ...
            try:
                with get_connection() as conn:
                    with conn.cursor() as cursor:
                        cursor.execute(query)
                snapshot = session.get("user_snapshot")
                if snapshot and snapshot.get("id") == user["id"]:
...
def admin_upload():
    ...
    return render_template("admin_upload.html", user=user, gallery=gallery, background_images=BACKGROUND_IMAGES)
